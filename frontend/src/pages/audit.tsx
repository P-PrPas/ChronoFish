...
export function Audit({ t = text.en }: { t?: AppText } = {}) {
  ...
  return <section><div className="page-heading"><div><p className="eyebrow">SCR-18 / APPEND-ONLY</p><h1>{t.audit}</h1><p className="muted">Filter every mutation by resource, record, operator, or time window.</p></div><button className="button button--secondary" onClick={() => void load()} disabled={loading}>{loading ? t.saving : t.refresh}</button></div><fieldset className="filter-bar"><legend>History filters</legend><label>Table<input value={filters.table ?? ''} onChange={(event) => update('table', event.target.value)} placeholder="batches" /></label><label>Record ID<input value={filters.recordId ?? ''} onChange={(event) => update('recordId', event.target.value)} /></label><label>Operator ID<input value={filters.operatorId ?? ''} onChange={(event) => update('operatorId', event.target.value)} /></label><label>From<input type="datetime-local" value={filters.from ?? ''} onChange={(event) => update('from', event.target.value)} /></label><label>To<input type="datetime-local" value={filters.to ?? ''} onChange={(event) => update('to', event.target.value)} /></label><button type="button" className="button button--secondary" onClick={() => setFilters({})}>Clear</button></fieldset>{error && <ErrorMessage message={error} />}{items.length === 0 ? <Empty message={t.empty} /> : <><div className="list">{items.map((item) => <details className="list-row" key={String(item.id)}><summary><span><strong>{String(item.action)} · {String(item.tableName)}</strong><small>{String(item.recordId)} · {String(item.occurredAt)}</small></span><span className="pill">{String(item.operatorName ?? item.operatorId ?? '—')}</span></summary><div className="audit-detail"><p><strong>Device:</strong> {String(item.deviceId ?? '—')}</p><pre>{JSON.stringify({ before: item.oldValues, after: item.newValues }, null, 2)}</pre></div></details>)}</div>{nextCursor && <button className="button button--secondary" onClick={() => void load(nextCursor, true)} disabled={loading}>{loading ? t.saving : t.refresh}</button>}</>}</section>
}
