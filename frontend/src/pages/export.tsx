...
export function PrintableDashboard({ filters }: { filters: DashboardFilters }) {
  ...
  return <section className="print-report" aria-labelledby="print-report-title"><div className="print-report__header"><p className="eyebrow">CHRONOFISH / DASHBOARD SUMMARY</p><h1 id="print-report-title">Experiment dashboard report</h1><p className="muted">Generated from the same filtered analytical dataset as the dashboard and workbook.</p></div>{report.loading && <p className="notice">Loading dashboard panels...</p>}{report.error && <ErrorMessage message={report.error} />}{!report.loading && !report.error && <>
    ...
    <ReportPanel title="Overview pipeline"><ReportTable headers={['Step', 'n', '% previous', '% activated']} rows={report.pipeline.map((point) => [String(point.step ?? '—'), Number(point.count ?? 0), `${(Number(point.pctOfPrevious ?? 0) * 100).toFixed(2)}%`, `${(Number(point.pctOfStart ?? 0) * 100).toFixed(2)}%`])} /></ReportPanel>
    ...
    <ReportPanel title="Attrition / abnormality onset"><ReportTable headers={['Stage', 'Count']} rows={report.abnormality.map((point) => [String(point.stageLabel ?? point.stageOrder ?? '—'), Number(point.count ?? 0)])} /></ReportPanel>
    <ReportPanel title="Timing deviation / group comparison"><ReportTable headers={['Stage', 'n', 'Mean H', 'Median H', 'SD H']} rows={report.deviation.map((point) => [String(point.stageLabel ?? point.stageOrder ?? '—'), Number(point.n ?? 0), Number(point.meanDeviationH ?? 0).toFixed(4), Number(point.medianDeviationH ?? 0).toFixed(4), point.sdDeviationH == null ? '—' : Number(point.sdDeviationH).toFixed(4)])} /></ReportPanel>
    ...
    <ReportPanel title="Control comparison"><ReportTable headers={['Normal %', 'Abnormal %']} rows={[[Number(stage1?.pctNormal ?? 0).toFixed(2), Number(stage1?.pctAbnormal ?? 0).toFixed(2)]]} /></ReportPanel>
