...
function App() {
  ...
  useEffect(() => {
    const refreshQueue = () => void Promise.all([queueCount(), rejectedQueueCount()]).then(([count, rejectedCount]) => { setPending(count); setRejected(rejectedCount); localStorage.setItem('chronofish.pending_count', String(count)) })
    const on = () => { setOnline(true); void drainQueue().then(refreshQueue) }
    const off = () => setOnline(false)
    const beforeClose = (event: BeforeUnloadEvent) => { if (Number(localStorage.getItem('chronofish.pending_count') ?? 0) > 0) { event.preventDefault(); event.returnValue = '' } }
