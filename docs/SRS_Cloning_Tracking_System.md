...
| ID | ข้อกำหนด | ระดับ |
|---|---|---|
| FR-401 | หน้าแรกต้องแสดง **ล็อตการฉีดที่ถึงรอบสังเกต** เรียงตามความเร่งด่วน โดย stage ที่ถึงเวลามากที่สุดเป็นเพียงคำแนะนำ ไม่ใช่ stage บังคับของทั้งล็อต | MUST |
| FR-402 | รายการต้องแสดง: `batch_code`, `lot_no`, stage ที่ระบบแนะนำ, และเวลาที่เลย/เหลือถึงกำหนด | MUST |
...
| ID | ข้อกำหนด | ระดับ |
|---|---|---|
| FR-405 | หน้าบันทึกรอบตรวจ lot ต้องแสดง**เฉพาะตัวอ่อนที่ยังไม่มี exit event** เท่านั้น | MUST |
| FR-406 | แต่ละตัวอ่อนต้องเริ่มด้วย stage ว่าง (ไม่บันทึกฟองนี้) และ outcome เริ่มต้น `ALIVE`; ระบบบันทึกเฉพาะฟองที่ผู้ใช้เลือก stage | MUST |
| FR-407 | แต่ละตัวอ่อนต้องเลือก stage, outcome (`ALIVE` / `DEAD` / `DEGENERATED` / `NOT_OBSERVED`) และ condition ได้อย่างอิสระด้วย dropdown | MUST |
| FR-408 | ต้องมีตัวเลือก stage ระดับรอบสำหรับนำไปใช้กับแถวที่ยังว่าง โดยยังแก้ stage รายฟองได้และไม่เขียนทับแถวที่เลือกแล้ว | MUST |
...
| FR-410 | ระบบต้องกำหนด `observed_at` เป็นเวลาปัจจุบันอัตโนมัติ ณ ขณะที่ผู้ใช้กดปุ่มยืนยันรอบตรวจ โดยไม่ใช้เวลาที่เปิดหน้า | MUST |
| FR-411 | ระบบต้องแสดงให้ชัดว่าเวลาจะถูกจับเมื่อกดยืนยัน และแสดงเวลาที่ยืนยันล่าสุดพร้อมสถานะการบันทึก | MUST |
| FR-412 | ระบบต้องคำนวณและเก็บส่วนต่างจากเวลามาตรฐานแยกตาม observation/stage ที่ผู้ใช้เลือก โดย**ไม่มีการตัดสินว่าปกติหรือผิดปกติ** | MUST |
| FR-413 | ระบบต้องบันทึกธง `is_backdated = true` อัตโนมัติเมื่อ observation ที่จับตอนยืนยันถูกส่งถึง server ช้ากว่า 15 นาที เช่นกรณีทำงาน offline | MUST |
| FR-414 | ระบบต้องให้ผู้ใช้ใส่ `notes` อิสระต่อ observation ได้ | MUST |
| FR-415 | ระบบต้องแสดงตำแหน่ง well ของแต่ละตัวอ่อนและจัดแบบฟอร์มรายฟองเป็น responsive grid ที่อ่านและแตะได้โดยไม่ต้องเลื่อนแนวนอน | SHOULD |
| FR-416 | ระบบต้องแสดงจำนวนรายการที่เลือกพร้อมยืนยันและจำนวนที่บันทึกแล้วในรอบปัจจุบัน | SHOULD |
| FR-417 | ตัวอ่อนในรอบตรวจเดียวกันต้องเลือก stage ต่างกันหรือเว้นไม่บันทึกบางฟองได้ และต้องอนุญาตให้ข้าม checkpoint ก่อนหน้า (ดู BR-08) | MUST |

> **AC-406** — กำหนดว่าล็อตมีตัวอ่อน 18 ตัว เมื่อผู้ใช้เลือกระยะ `4-cell` ให้ 15 ตัว, `8-cell` ให้ 2 ตัว และเว้น 1 ตัว แล้วกดยืนยัน ระบบต้องบันทึก 17 observations ด้วย stage ตามรายฟอง ใช้ `observed_at` เดียวกันจากเวลาที่ยืนยัน และไม่สร้างข้อมูลของฟองที่เว้นไว้
>
...
| FR-810 | Timing evidence | ตาราง `deviation_h` ต่อ checkpoint พร้อมจำนวนตัวอย่าง ค่าเฉลี่ย มัธยฐาน ต่ำสุด และสูงสุด; ไม่ใช้ box plot ในหน้าหลัก | SHOULD |
| FR-811 | Timing deviation เทียบกลุ่ม | ตารางเปรียบเทียบต่อ Treatment Group / Donor Cell Line ในส่วนวิเคราะห์เพิ่มเติมหรือไฟล์ส่งออก | SHOULD |
...
สำหรับทุกล็อตการฉีดที่ยังมีตัวอ่อนที่ไม่มี exit event ให้หา stage ล่าสุดที่บันทึกจริงของตัวอ่อนแต่ละฟอง แล้วใช้ลำดับต่ำสุดในล็อตเป็นฐานสำหรับคำแนะนำ:

```
latest_order(E) = ลำดับ stage สูงสุดที่บันทึกจริงของตัวอ่อน E (ถ้าไม่มี = 0)
completed_order(lot) = min(latest_order(E)) ของตัวอ่อนที่ยังติดตาม
สำหรับแต่ละ checkpoint k > completed_order(lot) ที่ stage_scope = STAGE_1 :
   due_at(k)  = activated_at + expected_hpa(k) ชั่วโมง
   ถึงกำหนด   เมื่อ now ≥ due_at(k)
   ความเร่งด่วน = now − due_at(k)      (มากกว่า = เร่งด่วนกว่า)
```
หน้า Due รวมผลเป็นหนึ่งรายการต่อ lot และใช้ checkpoint ที่เร่งด่วนที่สุดเป็นคำแนะนำเท่านั้น เพราะการบันทึกจริงเลือก stage แยกต่อฟองและอาจข้าม stage ได้
...
| ID | ข้อกำหนด | วิธีวัด |
|---|---|---|
| NFR-301 | รอบที่ตัวอ่อนทุกฟองอยู่ stage เดียวกันต้องบันทึกได้ภายใน **3 การแตะ** (เปิดหน้า → ใช้ stage ที่แนะนำกับแถวว่าง → ยืนยัน) | นับจาก UI flow |
...
| T-02 | บันทึกรอบที่ทั้ง 15 ฟองเป็น `2-cell` และไม่มีตัวไหนตาย | ใช้ 3 การแตะ · ได้ observation 15 รายการและเวลาตรงกับจังหวะยืนยัน | FR-406, NFR-301 |
| T-03 | รอบเดียวกันเลือก 13 ฟองเป็น `4-cell`, 2 ฟองเป็น `8-cell` และกำหนด 2 ฟองเป็น `DEAD` | ได้ stage รายฟองถูกต้อง · ตัวที่ตายหายจากหน้าถัดไปและมี exit event | FR-405, FR-407 |
| T-04 | บันทึกช้ากว่ากำหนด 25 นาที | แสดง `ช้ากว่าสากล 25 นาที` และเก็บ `deviation_h` ถูกต้อง | FR-412, BR-04 |
| T-05 | ในรอบเดียวกันบันทึก stage ต่างกันและเว้นบางฟองไว้ | ระบบบันทึกเฉพาะฟองที่เลือก · ยอมรับ stage ที่ข้ามและไม่สร้างข้อมูลแทนช่องที่เว้น | FR-417, BR-08 |
